...
func (rta *riskTypeAccess) CreateRiskType(ctx context.Context, tx *sql.Tx, riskType *interfaces.RiskType) error {
	...
	parametersBytes, _ := sonic.Marshal(riskType.Parameters)
	riskRulesBytes, _ := sonic.Marshal(riskType.RiskRules)
	riskFunctionBytes, _ := sonic.Marshal(riskType.RiskFunction)

	sqlStr, vals, err := sq.Insert(RT_TABLE_NAME).
		Columns(
			"f_id", "f_name", "f_description", "f_tags", "f_icon", "f_color", "f_comment",
			"f_kn_id", "f_branch", "f_max_acceptable_level", "f_parameters", "f_risk_rules", "f_risk_function",
		...
		Values(
			...
			riskType.KNID, riskType.Branch, riskType.MaxAcceptableLevel, parametersBytes, riskRulesBytes, riskFunctionBytes,
			riskType.Creator.ID, riskType.Creator.Type, riskType.CreateTime, riskType.Updater.ID, riskType.Updater.Type, riskType.UpdateTime,
		).
		ToSql()
	if err != nil {
		return err
...
func (rta *riskTypeAccess) ListRiskTypes(ctx context.Context, query interfaces.RiskTypesQueryParams) ([]*interfaces.RiskType, error) {
	...
	for rows.Next() {
		...
		var tagsStr string
		var parametersBytes, riskRulesBytes, riskFunctionBytes []byte

		err := rows.Scan(
			&rt.RTID, &rt.RTName, &rt.Comment, &tagsStr, &rt.Icon, &rt.Color, new(string),
			&rt.KNID, &rt.Branch, &rt.MaxAcceptableLevel, &parametersBytes, &riskRulesBytes, &riskFunctionBytes,
			&rt.Creator.ID, &rt.Creator.Type, &rt.CreateTime, &rt.Updater.ID, &rt.Updater.Type, &rt.UpdateTime,
		)
		if err != nil {
			return nil, err
		}
		...
		_ = sonic.Unmarshal(parametersBytes, &rt.Parameters)
		_ = sonic.Unmarshal(riskRulesBytes, &rt.RiskRules)
		_ = sonic.Unmarshal(riskFunctionBytes, &rt.RiskFunction)
...
func (rta *riskTypeAccess) UpdateRiskType(ctx context.Context, tx *sql.Tx, riskType *interfaces.RiskType) error {
	...
	data := map[string]any{
		...
		"f_description":          riskType.Comment,
		"f_tags":                 tagsStr,
		"f_icon":                 riskType.Icon,
		"f_color":                riskType.Color,
		"f_max_acceptable_level": riskType.MaxAcceptableLevel,
		"f_parameters":           parametersBytes,
		"f_risk_rules":           riskRulesBytes,
		"f_risk_function":        riskFunctionBytes,

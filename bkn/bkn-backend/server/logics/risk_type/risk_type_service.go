...
func (rts *riskTypeService) InsertDatasetData(ctx context.Context, riskTypes []*interfaces.RiskType) error {
	...
	for _, riskType := range riskTypes {
		...
		if params, exists := doc["parameters"]; exists {
			...
			if err != nil {
				logger.Errorf("Failed to marshal risk_type parameters: %s", err.Error())
				span.SetStatus(codes.Error, "序列化风险类参数失败")
				return err
			}
			doc["parameters"] = string(paramsBytes)
		}

		if riskRules, exists := doc["risk_rules"]; exists {
			riskRulesBytes, err := sonic.Marshal(riskRules)
			if err != nil {
				logger.Errorf("Failed to marshal risk_type risk_rules: %s", err.Error())
				span.SetStatus(codes.Error, "序列化风险类风险规则失败")
				return err
			}
			doc["risk_rules"] = string(riskRulesBytes)
		}

		if riskFunc, exists := doc["risk_function"]; exists && riskFunc != nil {
			riskFuncBytes, err := sonic.Marshal(riskFunc)
			if err != nil {
				logger.Errorf("Failed to marshal risk_type risk_function: %s", err.Error())
				span.SetStatus(codes.Error, "序列化风险类风险函数失败")
				return err
			}
			doc["risk_function"] = string(riskFuncBytes)
		}
		...
		for _, entry := range datasetResp.Entries {
			if paramsStr, exists := entry["parameters"]; exists {
				if paramsStrStr, ok := paramsStr.(string); ok && paramsStrStr != "" {
					var params []interfaces.ParamDef
					if err := sonic.Unmarshal([]byte(paramsStrStr), &params); err != nil {
						logger.Errorf("Failed to unmarshal risk_type parameters: %s", err.Error())
						return response, rest.NewHTTPError(ctx, http.StatusBadRequest,
			...
			if riskRulesStr, exists := entry["risk_rules"]; exists {
				if riskRulesStrStr, ok := riskRulesStr.(string); ok && riskRulesStrStr != "" {
					var riskRules []interfaces.RiskRule
					if err := sonic.Unmarshal([]byte(riskRulesStrStr), &riskRules); err != nil {
						logger.Errorf("Failed to unmarshal risk_type risk_rules: %s", err.Error())
						return response, rest.NewHTTPError(ctx, http.StatusBadRequest,
							berrors.BknBackend_InternalError_UnMarshalDataFailed).
							WithErrorDetails(fmt.Sprintf("failed to Unmarshal risk_rules, %s", err.Error()))
					}
					entry["risk_rules"] = riskRules
				}
			}

			if riskFuncStr, exists := entry["risk_function"]; exists {
				if riskFuncStrStr, ok := riskFuncStr.(string); ok && riskFuncStrStr != "" {
					var riskFunc interfaces.RiskFunction
					if err := sonic.Unmarshal([]byte(riskFuncStrStr), &riskFunc); err != nil {
						logger.Errorf("Failed to unmarshal risk_type risk_function: %s", err.Error())
						return response, rest.NewHTTPError(ctx, http.StatusBadRequest,
							berrors.BknBackend_InternalError_UnMarshalDataFailed).
							WithErrorDetails(fmt.Sprintf("failed to Unmarshal risk_function, %s", err.Error()))
					}
					entry["risk_function"] = &riskFunc
				} else if riskFuncStr == nil {
					entry["risk_function"] = nil
				}
			}

...
func (dca *dataConnectionAccess) TypeMapping(ctx context.Context, typeMappingReq *interfaces.TypeMappingReq) (*interfaces.TypeMappingResp, error) {
	ctx, span := ar_trace.Tracer.Start(ctx, "type mapping", trace.WithSpanKind(trace.SpanKindClient))
	defer span.End()

	urlStr := fmt.Sprintf("%s/api/internal/data-connection/v1/metadata/connectors/type/mapping", dca.appSetting.DataConnectionUrl)
	...
	// 序列化请求体
	reqBody, err := sonic.Marshal(typeMappingReq)
	if err != nil {
		logger.Errorf("DrivenMetadata TypeMapping marshal request failed: %v", err)

		o11y.AddHttpAttrs4Error(span, http.StatusInternalServerError, "InternalError", "Marshal request failed")
		o11y.Error(ctx, fmt.Sprintf("DrivenMetadata TypeMapping marshal request failed: %v", err))

		return nil, fmt.Errorf("DrivenMetadata TypeMapping marshal request failed: %v", err)
	}

	respCode, respData, err := dca.httpClient.PostNoUnmarshal(ctx, urlStr, headers, reqBody)
	logger.Debugf("post %s finished, response code is %d, result is %s, error is %v", urlStr, respCode, respData, err)

	if err != nil {
		logger.Errorf("DrivenMetadata TypeMapping request failed: %v", err)

		o11y.AddHttpAttrs4Error(span, respCode, "InternalError", "Http Post Failed")
		o11y.Error(ctx, fmt.Sprintf("DrivenMetadata TypeMapping request failed: %v", err))

		return nil, fmt.Errorf("DrivenMetadata TypeMapping request failed: %v", err)
	}
	...
	if respCode != http.StatusOK {
		...
		logger.Errorf("TypeMapping Error: %v", httpErr.Error())

		o11y.AddHttpAttrs4Error(span, respCode, "InternalError", "Http status is not 200")
		o11y.Error(ctx, fmt.Sprintf("TypeMapping failed: %v", vegaError))

		return nil, fmt.Errorf("TypeMapping Error: %v", httpErr.Error())
	}

	var res interfaces.TypeMappingResp
	if err = sonic.Unmarshal(respData, &res); err != nil {
		logger.Errorf("DrivenMetadata TypeMapping sonic.Unmarshal error: %v", err)

		o11y.AddHttpAttrs4Error(span, respCode, "InternalError", "Unmalshal TypeMappingResp failed")
		o11y.Error(ctx, fmt.Sprintf("Unmalshal TypeMappingResp failed: %v", err))

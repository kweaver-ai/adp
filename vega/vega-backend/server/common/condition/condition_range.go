...
func NewRangeCond(ctx context.Context, cfg *CondCfg, fieldsMap map[string]*interfaces.Property) (Condition, error) {
	if cfg.Name == "" {
	...
	if !dtype.DataType_IsDate(field.Type) && !dtype.DataType_IsNumber(field.Type) {
		return nil, fmt.Errorf("condition [range] left field is not a date field: %s:%s", cfg.Name, field.Type)
	}

...
func (ds *datasetService) Delete(ctx context.Context, res *interfaces.Resource) error {
	...
	if err != nil {
		...
		return rest.NewHTTPError(ctx, http.StatusInternalServerError, oerrors.VegaManager_Resource_InternalError).
			WithErrorDetails(err.Error())
	}
	if exist {
		// Delete from storage
...
func (ds *datasetService) ListDocuments(ctx context.Context, id string, params *interfaces.QueryParams) ([]map[string]any, int64, error) {
	...
	if err != nil {
		...
		return nil, 0, rest.NewHTTPError(ctx, http.StatusInternalServerError, oerrors.VegaManager_Resource_InternalError).
			WithErrorDetails(err.Error())
	}
...
func (ds *datasetService) CreateDocuments(ctx context.Context, id string, documents []map[string]any) ([]string, error) {
    ...
    if err != nil {
        ...
        return nil, rest.NewHTTPError(ctx, http.StatusInternalServerError, oerrors.VegaManager_Resource_InternalError_CreateFailed).
            WithErrorDetails(err.Error())
    }
...
func (ds *datasetService) GetDocument(ctx context.Context, id string, docID string) (map[string]any, error) {
	...
	if err != nil {
		...
		return nil, rest.NewHTTPError(ctx, http.StatusInternalServerError, oerrors.VegaManager_Resource_InternalError).
			WithErrorDetails(err.Error())
	}
